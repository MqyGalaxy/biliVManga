...
export default function ComicGallery({ initialData = [], initialHeaders = [] }) {
    ...
    // 修改公告版本号 (1)
    useEffect(() => {
        const hasSeen = localStorage.getItem('announcement_2026v7');
        if (!hasSeen) {
    ...
    const closeAnnouncement = () => {
        localStorage.setItem('announcement_2026v7', 'true');
                        ...
                        <div className="mb-8 space-y-4 text-sm font-medium text-gray-600 leading-relaxed overflow-y-auto pr-2">
                            ...
                            <ul className="space-y-2 pl-2 max-h-full overflow-y-auto">
                                <li className="flex items-start gap-2">
                                    ...
                                    <span>由于用于抓封面更新的脚本被b站风控412暂时无法工作，脚本现已更新完毕，目前已正常工作并完成了封面同步。</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="w-1.5 h-1.5 rounded-full bg-wata-pink mt-1.5 shrink-0"></span>
                                    <span>如果您发现了最近的封面有同步问题，请通过网页底部的“联系小站”联系我们。</span>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="w-1.5 h-1.5 rounded-full bg-wata-pink mt-1.5 shrink-0"></span>
                            ...
                            <p className="pt-2 text-xs text-gray-400"><br />更新日期：2026/09/05</p>
                        </div>
