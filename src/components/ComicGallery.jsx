...
export default function ComicGallery({ initialData = [], initialHeaders = [] }) {
    ...
    const filteredData = useMemo(() => {
        ...
        return validData.filter(item => {
            ...
            const rawUploader = getValue(item, ['译者昵称', '译者', 'UP主昵称', '转载up主', 'UP主', '上传']);
            const uploader = rawUploader.replace(/(^|[^a-zA-Z])[(（\[{【\s]*uid[:：\s]*\d+[)）\]}】\s]*/gi, '$1').toLowerCase().trim();
            
            return translatedName.includes(lowerSearch) || originalTitle.includes(lowerSearch) || author.includes(lowerSearch) || uploader.includes(lowerSearch);
        });
    }, [data, searchTerm]);
                ...
                <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button 
                            ...
                            disabled={loading}
                            className={`inline-flex items-center justify-center gap-1.5 px-4 py-2 sm:px-5 sm:py-2.5 rounded-full font-black text-xs sm:text-sm transition-all duration-300 shadow-wata hover:shadow-wata-hover hover:scale-105 ${loading ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-gradient-to-r from-wata-pink to-wata-purple text-white cursor-pointer'}`}
                        >
                            <UploadIcon />
                    ...
                    <div className="flex flex-wrap gap-2 mb-6 sm:mb-8 items-center">
                        ...
                        {searchTerm && (
                            <button 
                                ...
                                className="px-3 py-1 bg-gray-100 text-gray-500 text-[11px] sm:text-xs font-bold rounded-full hover:bg-gray-200 transition-all ml-auto cursor-pointer"
                            >
                                清除检索 x
                            </button>
                        )}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6">
                        ...
                        <div className="modal-enter relative bg-white rounded-[24px] sm:rounded-3xl shadow-2xl w-full max-w-3xl max-h-[92vh] sm:max-h-[90vh] flex flex-col overflow-hidden border-4 border-wata-lightPink">
                            ...
                            <div className="px-4 py-4 sm:px-6 sm:py-5 border-b border-gray-100 flex justify-between items-start relative shrink-0">
                                ...
                                <button onClick={() => setSelectedItem(null)} className="absolute right-3 top-3 sm:right-5 sm:top-5 text-gray-300 hover:text-wata-pink hover:rotate-90 bg-gray-50 hover:bg-wata-lightPink p-1.5 sm:p-2 rounded-full transition-all">
                                    <CloseIcon />
                                </button>
                            </div>
                            ...
                            <div className="px-4 py-3 sm:px-6 sm:py-4 bg-gray-50 border-t border-gray-100 flex justify-end shrink-0">
                                <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-2 sm:gap-3">
                                    {origLink && (
                                        <a href={origLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-6 py-2.5 bg-wata-purple text-white font-black rounded-full transition-all duration-300 shadow-wata hover:shadow-wata-hover hover:scale-105 cursor-pointer">
                                            <JumpLinkIcon /> 原视频链接
                                        </a>
                                    )}
                                    {biliLink && (
                                        <a href={biliLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-6 py-2.5 bg-[#fb7299] text-white font-black rounded-full transition-all duration-300 shadow-wata hover:shadow-wata-hover hover:scale-105 cursor-pointer">
                                            <BiliIcon /> 翻译视频链接
                                        </a>
                                    )}
                                    {(!biliLink && !origLink) && (
                                        <button onClick={() => setSelectedItem(null)} className="px-8 py-2 sm:py-2.5 bg-gray-200 text-gray-600 font-black rounded-full hover:bg-gray-300 text-sm">
                                            关闭
                                        </button>
                                    )}
                                </div>
